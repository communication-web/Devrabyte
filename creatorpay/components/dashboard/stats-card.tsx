...
export function StatsCard({ label, amount, icon: Icon, color, subtitle }: StatsCardProps) {
  ...
  return (
    <div className="relative bg-white rounded-xl border border-gray-100 p-5 overflow-hidden">
      <div className={`absolute inset-0 bg-gradient-to-br ${c.accent} to-transparent opacity-50 pointer-events-none`} />
      <div className="relative flex items-start justify-between">
        <div className="flex-1">
          <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-3">{label}</p>
          <p className="text-2xl font-bold text-gray-900 leading-none">{formatCurrency(amount)}</p>
